...
const Skills = () => {
  ...
  const skillCategories = {
    ...
    'Backend': [
      ...
      skills.find(s => s.name === 'Computer Networking'),
    ].filter(Boolean),
    'Tools & Others': [
  ...
  const getLevelColor = (level) => {
    const colors = {
      ...
      'Intermediate': 'text-emerald-400 bg-emerald-500/20 border-gray-500/30',
    };
                    ...
                    return (
                      <div key={skill.id} className="space-y-2">
                        <div className="flex items-center justify-between">
                          ...
                          <span className={`text-xs px-2 py-1 rounded border ${getLevelColor(skill.level)}`}>
                            {skill.level}
                          </span>
                        </div>
